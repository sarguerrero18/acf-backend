...
import { AlmIngresoCabecera, AlmIngresoDetalleLinea } from '../repositorios/almActasRepo';

export function generarActaAlmIngreso(
  ...
  return new Promise((resolve, reject) => {
    ...
    dibujarTabla(doc, [
      { titulo: 'Elemento', ancho: 220, valor: (f: AlmIngresoDetalleLinea) => f.descripcion },
      { titulo: 'Cantidad', ancho: 70, align: 'right', valor: (f: AlmIngresoDetalleLinea) => String(f.cantidad) },
      { titulo: 'Unidad', ancho: 70, valor: (f: AlmIngresoDetalleLinea) => f.unidad_medida },
      { titulo: 'Valor unitario', ancho: 76, align: 'right', valor: (f: AlmIngresoDetalleLinea) => `$${fmtValor(f.valor_unitario)}` },
      { titulo: 'Valor total', ancho: 76, align: 'right', valor: (f: AlmIngresoDetalleLinea) => `$${fmtValor(f.valor_total)}` },
    ], detalle);
